...
class User(object):
    ...
    def _load_properties(self):
        ...
        if int(person.iconserver) > 0:
            self.__icon_url = 'http://photos%s.flickr.com/%s/buddyicons/%s.jpg' \
                              % (person.iconfarm, person.iconserver, self.__id)
        else:
...
def _doget(method, auth=False, **params):
    ...
    return _get_data(minidom.parseString(fetch(url).content))
